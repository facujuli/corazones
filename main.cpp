...
float getEntropy(CSVData data, map <int, string> attributes);
...
int main(void)
{
    ...
    map<int, string> attributes = {{1, "1"}, {3, "1"}};

    float entropia= getEntropy(heart, attributes);
    cout << entropia << endl;
...
float getEntropy(CSVData data, map<int, string> attributes)
{
    ...
    for(j = 0; j < last; j++)
    {
        ...
        for(i = 0; i < 13; i++)
        {
            auto it = attributes.find(i);     //verifico si el atributo de la instancia coincide con alguno que debo fijar
